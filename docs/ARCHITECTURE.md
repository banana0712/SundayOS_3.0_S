...
├──────────────────────────────────────────────────────────────┤
│ L4 交互人格   共情计算 · 人格锚定 · 对话管理 ✅                 │
├──────────────────────────────────────────────────────────────┤
│ L3 认知引擎   双系统(Talker+Reasoner) · ReAct · 规划器 ✅       │
├──────────────────────────────────────────────────────────────┤
│ L1.5 引擎抽象  ★ 统一 Provider 接口 · 动态路由器 · 回退链 ✅  │
│               豆包(primary)│DeepSeek│Qwen│Claude│GPT│Ollama   │
├──────────────────────────────────────────────────────────────┤
│ L2 数据记忆   Storage 层 ✅ → Reflection 层 ✅ → Experience 层 ✅│
│               + 上下文窗口压缩 ✅                               │
├──────────────────────────────────────────────────────────────┤
│ L1 基础设施   SQLite ✅ + 语义 embedding(Qwen) ✅ + 进程内缓存  │
│               GitHub 真源（未实现）                             │
└──────────────────────────────────────────────────────────────┘
```

**实现状态**（v0.10.9 更新）：
- L1.5 引擎抽象层：✅ 动态路由 + 质量优先 + 豆包模型选择优化
- L2 记忆三层：✅ Storage + Reflection + Experience + SQLite 持久化 + 上下文压缩
- L3 双系统：✅ Talker/Reasoner 切换 + ReAct 循环 + 工具执行
- L4 交互人格：✅ 共情计算 + 人格锚定 + 反馈学习
- L5 应用技能：✅ 8 技能注册 + 工具路由 + 护栏门控
- L6 安全治理：✅ 六层护栏 + PII 脱敏 + 注入拦截
- L1 基础设施：✅ SQLite 持久化 + Qwen 语义 embedding
