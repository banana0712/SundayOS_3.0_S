...
### Added
- **上下文窗口管理器** (`context_window.py`)：
  - 智能对话压缩：超过12条消息自动触发
  - LLM驱动的摘要生成（fallback到简单截断摘要）
  - 关键事实提取并自动存入记忆系统
  - 滑动窗口策略：保留最近6条消息完整，压缩更早历史
  - 压缩比统计和详细指标记录
  - 测试验证：26条消息压缩到10条，摘要正常存储和使用
  - ✅ 已部署到生产环境并验证通过
...
- **Chat 端点优化**：
  ...
  - 对话历史自动注入到引擎消息列表
  - 压缩摘要在下一轮对话中作为系统提示注入
  - 使用 `ctx.runtime` 替代模块级 runtime 引用
- **数据库结构升级**：
  - `conversations` 表新增 `summary` 字段存储压缩摘要
  - 自动迁移支持（ALTER TABLE）
  - SQLiteConversationStore 支持 summary 的持久化
- **全局异常处理**：
  - 添加全局异常处理器，捕获完整错误堆栈
  - 启用详细日志记录（DEBUG级别）
...
### Fixed
- **工具调试修复** (v0.10.7)：
  - ReAct 工具参数解析优化，支持自然语言输入
  - 修复 `create_reminder` 的中文时间识别
  - 修复多个导入错误：`log` → `log_engine`, `memory.base` → `memory.schema`
  - 创建缺失的 `guardrails/pii.py` 占位模块
- **消息格式统一**：
  - 修复 `_get_recent_topics` 中 `m.role` → `m.get("role")` 的类型错误
  - 在 main.py 和 chat.py 中同步修复
  - 清理所有调试代码
- **端口冲突问题**：
  - 解决8000端口多进程冲突导致的500错误
  - 根因：多个 uvicorn 进程同时运行导致请求路由混乱
...
### Changed
- **web_search 工具优化**：
  - 添加代理支持（从环境变量读取 SEARCH_PROXY/HTTPS_PROXY/HTTP_PROXY）
  - 结构化结果输出（标题 + URL + 摘要，之前仅摘要）
  - 使用异步 httpx 替代同步请求（非阻塞）
  - 增加超时时间（10s → 20s）
  - 支持 duckduckgo-search 库（优先）+ HTML 解析（降级）
  - 网络错误时给出清晰的代理配置提示
  - 结果数量优化（8 → 5 条，更聚焦）

### Fixed
- 修复 DuckDuckGo 被墙时的错误提示不清晰问题
- 修复搜索结果缺少来源 URL 的问题
